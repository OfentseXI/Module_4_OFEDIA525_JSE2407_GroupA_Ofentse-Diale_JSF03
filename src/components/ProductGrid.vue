<template>
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
    <div v-for="product in products" :key="product.id" class="card-container bg-white shadow-md rounded-lg overflow-hidden border p-4 cursor-pointer hover:border-black shadow-lg transition-shadow duration-300 flex flex-col h-full">
      ...
      <div class="card-content p-4 flex flex-col flex-grow">
        ...
        <div class="mt-auto flex justify-evenly items-center">
          ...
          <button class="bg-blue-900 hover:bg-blue-300 text-white font-bold py-2 px-4 rounded focus:outline-none focus:ring-2 focus:ring-pink-400 focus:ring-opacity-75 transition duration-200">
            Add To Cart
          </button>
        </div>
      </div>
